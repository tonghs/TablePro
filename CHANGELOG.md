...
- Vim mode: full standard command surface in the SQL editor. Motions (h/j/k/l, w/W/b/B/e/E/ge/gE, 0/$/^/_, gg/G with first-non-blank, H/M/L, %, f/F/t/T with ;/, repeat, ( ) sentence, { } paragraph, [[ ]] section). Operators (d/c/y with motions, D/C/Y/S/s/r/R, ~/gu/gU/g~ case with line forms, >>/<< indent). Text objects (iw/aw, iW/aW, i"/a", i'/a', i(/a(, i{/a{, i[/a[, i</a<, it/at, ip/ap). Search (/?nN*# with whole-word matching and wrap-around). Marks (m/'/`) including '< and '> for the last visual selection. Named registers "a-z, numbered registers "0-9 with the delete rotation ring, uppercase append. Macros (q/@/@@ with count). Insert-mode editing (Ctrl+W/U/H/T/D). Number adjust (Ctrl+A/X with count, negatives, hex). Scroll (Ctrl+d/u/f/b, Ctrl+e/y, zt/zz/zb, gj/gk). The repeat command (.) and the line undo command (U). The gv command reselects the last visual selection
...
- Vim mode: Shift+J now joins lines as expected. The editor's Vim engine ignored the J command, so users could not collapse multi-line queries. The new implementation matches vim semantics: J inserts a single space at the join, gJ joins without a space, count prefixes like 3J join three lines, and pressing J in visual mode joins every selected line (#1222)
- Vim mode: pressing Esc no longer leaves the editor stuck in INSERT after typing `;` at the end of a query. The popup-close observer was racing with `NSApp.currentEvent` so the engine never saw Esc, and the SwiftUI Clear Selection menu shortcut was preempting the local key monitor. Esc now reaches the engine through both paths and always switches mode
- Vim mode: `w` and `W` no longer leave a block-cursor highlight past the last char of the buffer. The motion now clamps to a valid normal-mode cursor position when there is no next word, matching vim's last-word-on-last-line rule
